...
which is that the `id` value is generated when the `NewEvent` class is instantiated.

We can see that the second call to `append_events()` returns successfully. We
can also see the stream has been unchanged despite calling the append_events()
